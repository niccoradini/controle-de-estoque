...
function stockCountMetricsMarkup(summary, updatedAt) {
  return `<article><small>Produtos contados</small><strong>${summary.countedProducts}</strong><span>de ${summary.products}</span></article><article><small>Unidades contadas</small><strong>${summary.countedUnits}</strong></article><article><small>Ainda não contados</small><strong>${summary.remainingProducts}</strong></article><article class="is-alert"><small>Com divergência</small><strong>${summary.divergentProducts}</strong></article><article class="is-progress"><small>Concluído</small><strong>${summary.progress}%</strong><span>${escapeHtml(formatDate(updatedAt))}</span></article>`;
}

function stockCountScanMarkup(scan) {
  return `<div class="stock-count-quick__success" role="status">${uiIcon('check')}<div><strong>${escapeHtml(scan.productName)}</strong><span>${escapeHtml(scan.materialCode)}${scan.type === 'material' ? ` · agora ${scan.countedQuantity} unidade(s)` : ' · IMEI/serial registrado'}</span></div></div>`;
}

function applyStockCountScan(result) {
  const details = state.stockCount;
  const item = details.items.find((entry) => entry.variantId === result.item.variantId);
  if (!item) return openStockCount(details.count.id);
  Object.assign(item, result.item);
  if (result.serial) {
    const serial = details.serials.find((entry) => entry.serialNumber.toUpperCase() === result.serial.serialNumber.toUpperCase());
    if (serial) Object.assign(serial, result.serial);
    else details.serials.push(result.serial);
  }
  details.count.updatedAt = result.updatedAt;
  details.summary = {
    products: details.items.length,
    countedProducts: details.items.filter((entry) => entry.countedQuantity != null).length,
    countedUnits: details.items.reduce((total, entry) => total + (entry.countedQuantity || 0), 0),
    divergentProducts: details.items.filter((entry) => entry.difference != null && entry.difference !== 0).length,
  };
  details.summary.remainingProducts = details.summary.products - details.summary.countedProducts;
  details.summary.progress = details.summary.products ? Math.round(details.summary.countedProducts / details.summary.products * 100) : 100;
  state.stockCountLastScan = result.scan;
  if (state.stockCountSearch || state.stockCountFilter !== 'all') {
    renderStockCountWorkspace();
  } else {
    const metrics = document.querySelector('.stock-count-metrics');
    if (metrics) metrics.innerHTML = stockCountMetricsMarkup(details.summary, result.updatedAt);
    const card = document.querySelector(`[data-stock-count-item="${item.variantId}"]`);
    if (card) card.outerHTML = stockCountProductCard(item);
    const success = document.querySelector('.stock-count-quick__success');
    if (success) success.outerHTML = stockCountScanMarkup(result.scan);
    else document.querySelector('.stock-count-quick')?.insertAdjacentHTML('beforeend', stockCountScanMarkup(result.scan));
  }
  const input = document.querySelector('[data-stock-count-quick-input]');
...
let stockCountQuickInFlight = false;
let stockCountLastSubmitted = { code: '', at: 0 };
...
function productCard(product) {
  ...
  const price = selectedProductPrice(product, variant);
  const payment = price == null || price <= 0 ? null : paymentPriceLines(price);
  const priceDetail = price == null
    ...
    : productPriceKind(product, variant) === 'no_charge'
      ...
      : `<strong>${formatMoney(price)} à vista</strong><span><b>PIX</b> ${formatMoney(payment.pix)}</span><span><b>12x</b> ${formatMoney(payment.twelve)} sem juros</span><span><b>${payment.longCount}x</b> ${formatMoney(payment.longInstallment)} · total ${formatMoney(payment.longTotal)}</span>`;
...
function selectedDevicePrice(product, variant = null) {
  if (!product?.pricing) {
    ...
    const payment = paymentPriceLines(retailPrice);
    return `<div class="device-selected-price"><span>Preço do produto</span><strong>${formatMoney(retailPrice)}</strong><div class="device-selected-price__options"><small>PIX ${formatMoney(payment.pix)}</small><small>12x ${formatMoney(payment.twelve)} sem juros</small><small>${payment.longCount}x ${formatMoney(payment.longInstallment)} · total ${formatMoney(payment.longTotal)}</small></div></div>`;
  }
  ...
  const payment = paymentPriceLines(price);
  return `<div class="device-selected-price"><span>${escapeHtml(state.priceCategory)}</span><strong>${formatMoney(price)}</strong><div class="device-selected-price__options"><small>PIX ${formatMoney(payment.pix)}</small><small>12x ${formatMoney(payment.twelve)} sem juros</small><small>${payment.longCount}x ${formatMoney(payment.longInstallment)} · total ${formatMoney(payment.longTotal)}</small></div></div>`;
}
...
function pricingSelector() {
  ...
  return `<section class="pricing-selector"><div><span class="pricing-selector__eyebrow">Tabela de preços</span><h3>Categoria do plano</h3><p>Selecione para ver o valor exato dos aparelhos.</p></div><div class="pricing-selector__control"><label for="pricing-category">Categoria</label><select class="select" id="pricing-category" data-action="pricing-category"><option value="">Selecione o plano…</option>${categories.map((category) => `<option value="${escapeHtml(category)}" ${state.priceCategory === category ? 'selected' : ''}>${escapeHtml(category)}</option>`).join('')}</select><small>${escapeHtml(state.pricing.source || 'Tabela de preços')} · ${escapeHtml(tableDate)}</small></div></section><section class="payment-policy-guide" aria-label="Nova política de pagamento"><header><div><span>Nova política · vigente desde ${escapeHtml(formatDateOnly(policy.effectiveDate))}</span><h3>Como oferecer as formas de pagamento</h3></div><b>Até ${Number(policy.maxInstallments)}x</b></header><div><article><strong>PIX / Vivo Pay</strong><span>10% menor que o preço-base.</span></article><article><strong>1x a 12x</strong><span>Mesmo total do preço-base, sem acréscimo.</span></article><article><strong>13x a 21x</strong><span>Total com acréscimo progressivo, calculado automaticamente.</span></article><article><strong>Venda flexível</strong><span>Combine até 3 meios de pagamento e aplique o Vivo Renova quando elegível.</span></article></div><p>O limite disponível depende da elegibilidade do cartão. Sempre confirme o total calculado antes de concluir a venda.</p></section>`;
}
...
function renderCatalogGrid() {
  ...
  const inventoryTarget = document.querySelector('[data-store-inventory]');
  if (inventoryTarget) inventoryTarget.innerHTML = sellerStockInventoryOverview();
}

function sellerStockInventoryOverview() {
  const products = filteredCatalog();
  const rows = products.flatMap((product) => product.variants.map((variant) => ({ product, variant })));
  const totals = rows.reduce((summary, { variant }) => ({
    onHand: summary.onHand + Number(variant.onHand || 0),
    reserved: summary.reserved + Number(variant.reserved || 0),
    available: summary.available + Number(variant.available || 0),
    incoming: summary.incoming + Number(variant.incoming || 0),
  }), { onHand: 0, reserved: 0, available: 0, incoming: 0 });
  const groupedRows = clusterOrder.map((cluster) => ({
    cluster,
    rows: rows.filter(({ product }) => product.cluster === cluster),
  })).filter((group) => group.rows.length);
  const content = groupedRows.map((group) => `<section class="store-inventory-group"><header><strong>${escapeHtml(clusterLabels[group.cluster])}</strong><span>${group.rows.length} ${group.rows.length === 1 ? 'material' : 'materiais'}</span></header><div>${group.rows.map(({ product, variant }) => `<article class="store-inventory-row">
      <div class="store-inventory-row__product">${productImageMarkup(product, 'store-inventory-row__image', 46, 46)}<span><strong>${escapeHtml(product.name)}</strong><code class="mono">${escapeHtml(variant.materialCode || '—')}</code></span></div>
      <span class="store-inventory-number"><small>Físico</small><b>${Number(variant.onHand || 0)}</b></span>
      <span class="store-inventory-number"><small>Reservado</small><b>${Number(variant.reserved || 0)}</b></span>
      <span class="store-inventory-number is-available"><small>Disponível</small><b>${Number(variant.available || 0)}</b></span>
      <span class="store-inventory-number is-incoming"><small>Em chegada</small><b>${Number(variant.incoming || 0)}</b></span>
    </article>`).join('')}</div></section>`).join('');
  return `<div class="store-inventory-summary"><span><small>Saldo físico</small><strong>${totals.onHand}</strong></span><span><small>Reservado</small><strong>${totals.reserved}</strong></span><span><small>Disponível</small><strong>${totals.available}</strong></span><span><small>Em chegada</small><strong>${totals.incoming}</strong></span></div>${content || emptyState('Nenhum material encontrado', 'Altere a pesquisa ou o grupo selecionado.')}`;
}
...
function paymentOptionsAlignment() {
  ...
  return `<div class="alignment-detail__body alignment-payment-morning">
    ...
    <section class="alignment-payment-grid" aria-label="Três opções de pagamento">
      <article class="alignment-payment-card is-pix"><span>01 · Menor valor total</span><h4>PIX ou Vivo Pay</h4><strong>10% a menos</strong><p>Funciona em qualquer produto e plano. Use como argumento decisivo quando economia for a prioridade.</p></article>
      <article class="alignment-payment-card"><span>02 · Equilíbrio</span><h4>De 1x a 12x</h4><strong>Preço-base</strong><p>O total permanece igual ao preço-base. Apresente parcela e total lado a lado.</p></article>
      <article class="alignment-payment-card is-long"><span>03 · Parcela menor</span><h4>De 13x a 21x</h4><strong>Mais possibilidades</strong><p>Disponível em cartões selecionados. O simulador informa parcela e total para cada prazo.</p></article>
    </section>
    ${paymentSurchargeTable()}
    <div class="alignment-detail-grid">
      <section class="alignment-panel"><div class="alignment-panel__heading"><div><span class="alignment-panel__label">Roteiro de negociação</span><h4>Da descoberta ao fechamento.</h4></div><span class="alignment-count">5 passos</span></div><div class="alignment-process">${approaches.map(([title, text], index) => `<article><span>${index + 1}</span><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(text)}</p></div></article>`).join('')}</div></section>
...
function paymentSurchargeTable() {
  const exampleCents = 100000;
  const rates = paymentPolicy().installmentSurchargePartsPerMillion || {};
  const rows = Array.from({ length: 9 }, (_, index) => index + 13).map((installments) => {
    const rate = Number(rates[installments] || 0) / 10000;
    const total = installmentTotalCents(exampleCents, installments);
    const installment = Math.round(total / installments);
    return `<tr><th scope="row">${installments}x</th><td>+${rate.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%</td><td>${formatMoney(total)}</td><td>${formatMoney(installment)}</td></tr>`;
  }).join('');
  return `<section class="alignment-rate-table" aria-labelledby="alignment-rate-title"><header><div><span>Consulta rápida</span><h4 id="alignment-rate-title">Como o acréscimo evolui de 13x a 21x</h4></div><p>Exemplo calculado sobre uma compra de R$ 1.000,00. Para a venda real, confirme sempre os números do simulador.</p></header><div class="alignment-rate-table__scroll"><table><thead><tr><th>Prazo</th><th>Acréscimo no total</th><th>Total no exemplo</th><th>Parcela aproximada</th></tr></thead><tbody>${rows}</tbody></table></div><footer>Quanto maior o prazo, menor fica a parcela e maior fica o valor total da compra.</footer></section>`;
}
...
function alignmentDetail() {
  ...
  const bodies = {
    'payment-options': paymentOptionsAlignment,
...
async function renderIncoming() {
  ...
  const cards = data.products.map((product) => {
    const dateLabel = product.firstDeliveryOn === product.lastDeliveryOn
      ? formatDateOnly(product.firstDeliveryOn)
      : `${formatDateOnly(product.firstDeliveryOn)} — ${formatDateOnly(product.lastDeliveryOn)}`;
    const serialRows = product.serials.map((serial) => `<tr><td><code class="mono">${escapeHtml(serial.serialNumber)}</code></td><td><strong class="incoming-delivery-date">${escapeHtml(formatDateOnly(serial.deliveryStartedOn))}</strong></td><td>${escapeHtml(serial.center)}</td><td>${escapeHtml(serial.deposit || 'NREM')}</td><td>${escapeHtml(serial.stockType)}</td><td><span class="incoming-status-dot">${escapeHtml(serial.status)}</span></td></tr>`).join('');
    ...
    return `<article class="incoming-card" data-incoming-product data-search="${escapeHtml(searchText)}">
      ...
      <div class="incoming-card__body"><div class="incoming-card__top"><span>${escapeHtml(product.brand || 'Produto')}</span><strong><b>${Number(product.quantity || product.serials.length)}</b> ${Number(product.quantity || product.serials.length) === 1 ? 'unidade' : 'unidades'}</strong></div><h3>${escapeHtml(product.name)}</h3><p>${escapeHtml(product.technicalName || 'Produto identificado no relatório de estoque')}</p><div class="incoming-card__delivery">${uiIcon('history')}<span>Colocado em entrega<strong>${escapeHtml(dateLabel)}</strong></span></div><div class="incoming-card__meta"><code class="mono">${escapeHtml(product.materialCode)}</code><span>Em deslocamento</span></div></div>
      <details class="incoming-card__details"><summary><span>Ver séries e detalhes</span>${uiIcon('chevron')}</summary><div class="incoming-card__table-wrap"><table><thead><tr><th>Série / IMEI</th><th>Em entrega desde</th><th>Centro</th><th>Depósito</th><th>Tipo</th><th>Status</th></tr></thead><tbody>${serialRows}</tbody></table></div></details>
    </article>`;
  }).join('');
...
function printSelectedLabels() {
  ...
  portal.classList.add(state.labelMode === 'store' ? 'label-print-portal--store' : 'label-print-portal--cases');
...
document.addEventListener('submit', async (event) => {
  ...
  const quickScan = form.dataset.form === 'stock-count-quick';
  const quickCode = quickScan ? String(new FormData(form).get('code') || '').trim().toUpperCase() : '';
  if (quickScan) {
    if (stockCountQuickInFlight || (stockCountLastSubmitted.code === quickCode && Date.now() - stockCountLastSubmitted.at < 750)) return;
    stockCountQuickInFlight = true;
    stockCountLastSubmitted = { code: quickCode, at: Date.now() };
  }
  setFormError(form);
  const submit = form.querySelector('button[type="submit"]');
  ...
  }); } finally { if (quickScan) stockCountQuickInFlight = false; }
});
