...
function renovaSearchResultsWorkbook(items, generatedOn) {
  ...
  const merges = [
    'A2:J2', 'A3:J3',
    'A5:C5', 'D5:F5', 'G5:H5', 'I5:J5',
    'A7:J7',
  ];
  const results = items.map((item, index) => {
    const mainRow = 9 + (index * 3);
    const detailRow = mainRow + 1;
    const spacerRow = mainRow + 2;
    const status = item.pickup_on ? 'Retirado' : 'Aguardando retirada';
    const statusStyle = item.pickup_on ? 12 : 11;
    const mainStyle = 10;
    const receivedOn = String(item.received_on || '').split('-').reverse().join('/');
    const pickupOn = item.pickup_on ? String(item.pickup_on).split('-').reverse().join('/') : 'Permanece na loja';
    const createdBy = item.created_by_name || 'Equipe da loja';
    const updatedBy = item.updated_by_name || createdBy;
    merges.push(
      `B${mainRow}:D${mainRow}`, `E${mainRow}:G${mainRow}`, `H${mainRow}:J${mainRow}`,
      `A${detailRow}:D${detailRow}`, `E${detailRow}:G${detailRow}`, `H${detailRow}:J${detailRow}`,
    );
    return `<row r="${mainRow}" ht="29"><c r="A${mainRow}" s="9" t="inlineStr"><is><t>${xmlCell(item.registration_code || '—')}</t></is></c><c r="B${mainRow}" s="${mainStyle}" t="inlineStr"><is><t>${xmlCell(item.model)}</t></is></c><c r="E${mainRow}" s="${mainStyle}" t="inlineStr"><is><t>${xmlCell(item.imei || 'IMEI não informado')}</t></is></c><c r="H${mainRow}" s="${statusStyle}" t="inlineStr"><is><t>${status}</t></is></c></row><row r="${detailRow}" ht="24"><c r="A${detailRow}" s="13" t="inlineStr"><is><t>Recebido ${receivedOn}   ·   Cadastrado ${xmlCell(renovaWorkbookDateTime(item.created_at))}</t></is></c><c r="E${detailRow}" s="13" t="inlineStr"><is><t>${item.pickup_on ? `Retirado ${pickupOn}` : pickupOn}</t></is></c><c r="H${detailRow}" s="14" t="inlineStr"><is><t>${xmlCell(createdBy)}   ·   Atualizado ${xmlCell(renovaWorkbookDateTime(item.updated_at))} por ${xmlCell(updatedBy)}</t></is></c></row><row r="${spacerRow}" ht="7"/>`;
  }).join('');
  ...
  const lastRow = items.length ? 8 + (items.length * 3) : 9;
  const mergedCells = merges.map((ref) => `<mergeCell ref="${ref}"/>`).join('');
  const sheet = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:J${lastRow}"/><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="7" topLeftCell="A8" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="17"/><cols><col min="1" max="1" width="11" customWidth="1"/><col min="2" max="7" width="15" customWidth="1"/><col min="8" max="10" width="18" customWidth="1"/></cols><sheetData><row r="1" ht="12"/><row r="2" ht="30"><c r="A2" s="1" t="inlineStr"><is><t>Resultado Renova</t></is></c></row><row r="3" ht="22"><c r="A3" s="2" t="inlineStr"><is><t>Histórico de recebimentos e retiradas de aparelhos</t></is></c></row><row r="5" ht="30"><c r="A5" s="4" t="inlineStr"><is><t>${items.length} cadastrados</t></is></c><c r="D5" s="5" t="inlineStr"><is><t>${awaiting} aguardando retirada</t></is></c><c r="G5" s="6" t="inlineStr"><is><t>${pickedUp} retirados</t></is></c><c r="I5" s="7" t="inlineStr"><is><t>Gerado ${xmlCell(generatedOn)}</t></is></c></row><row r="7" ht="24"><c r="A7" s="7" t="inlineStr"><is><t>${items.length} ${items.length === 1 ? 'aparelho encontrado' : 'aparelhos encontrados'}   ·   mais recentes primeiro</t></is></c></row>${results}${emptyRow}</sheetData><mergeCells count="${merges.length}">${mergedCells}</mergeCells><pageMargins left="0.3" right="0.3" top="0.35" bottom="0.35" header="0.2" footer="0.2"/><pageSetup orientation="landscape" paperSize="9" fitToWidth="1" fitToHeight="0"/></worksheet>`;
