...
function renovaSearchResultsWorkbook(items, generatedOn) {
  const awaiting = items.filter((item) => !item.pickup_on).length;
  const pickedUp = items.length - awaiting;
  const merges = [
    'A1:J1', 'A2:J2',
    'A4:B4', 'D4:E4', 'G4:H4', 'I4:J4',
    'A5:B5', 'D5:E5', 'G5:H5', 'I5:J5',
    'A7:J7',
  ];
  const cards = items.map((item, index) => {
    const topRow = 9 + (index * 4);
    const detailRow = topRow + 1;
    const auditRow = topRow + 2;
    const spacerRow = topRow + 3;
    const status = item.pickup_on ? 'RETIRADO' : 'AGUARDANDO RETIRADA';
    const statusStyle = item.pickup_on ? 12 : 11;
    const receivedOn = String(item.received_on || '').split('-').reverse().join('/');
    const pickupOn = item.pickup_on ? String(item.pickup_on).split('-').reverse().join('/') : 'Ainda na loja';
    merges.push(
      `A${topRow}:B${topRow}`, `C${topRow}:G${topRow}`, `H${topRow}:J${topRow}`,
      `A${detailRow}:C${detailRow}`, `D${detailRow}:E${detailRow}`,
      `F${detailRow}:G${detailRow}`, `H${detailRow}:J${detailRow}`,
      `A${auditRow}:C${auditRow}`, `D${auditRow}:F${auditRow}`, `G${auditRow}:J${auditRow}`,
    );
    return `<row r="${topRow}" ht="34"><c r="A${topRow}" s="9" t="inlineStr"><is><t>${xmlCell(item.registration_code || 'SEM CÓDIGO')}</t></is></c><c r="C${topRow}" s="10" t="inlineStr"><is><t>${xmlCell(item.model)}</t></is></c><c r="H${topRow}" s="${statusStyle}" t="inlineStr"><is><t>${status}</t></is></c></row><row r="${detailRow}" ht="27"><c r="A${detailRow}" s="13" t="inlineStr"><is><t>IMEI  ${xmlCell(item.imei || 'Não informado')}</t></is></c><c r="D${detailRow}" s="13" t="inlineStr"><is><t>RECEBIDO  ${receivedOn}</t></is></c><c r="F${detailRow}" s="13" t="inlineStr"><is><t>RETIRADO  ${pickupOn}</t></is></c><c r="H${detailRow}" s="13" t="inlineStr"><is><t>CADASTRADO POR  ${xmlCell(item.created_by_name || 'Equipe da loja')}</t></is></c></row><row r="${auditRow}" ht="25"><c r="A${auditRow}" s="14" t="inlineStr"><is><t>CADASTRADO EM  ${xmlCell(renovaWorkbookDateTime(item.created_at))}</t></is></c><c r="D${auditRow}" s="14" t="inlineStr"><is><t>ATUALIZAÇÃO  ${xmlCell(renovaWorkbookDateTime(item.updated_at))}</t></is></c><c r="G${auditRow}" s="14" t="inlineStr"><is><t>ATUALIZADO POR  ${xmlCell(item.updated_by_name || item.created_by_name || 'Equipe da loja')}</t></is></c></row><row r="${spacerRow}" ht="8"/>`;
  }).join('');
  const emptyRow = items.length ? '' : '<row r="9" ht="42"><c r="A9" s="15" t="inlineStr"><is><t>Nenhum aparelho cadastrado no Renova.</t></is></c></row>';
  if (!items.length) merges.push('A9:J9');
  const lastRow = items.length ? 8 + (items.length * 4) : 9;
  const mergedCells = merges.map((ref) => `<mergeCell ref="${ref}"/>`).join('');
  const sheet = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><dimension ref="A1:J${lastRow}"/><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="7" topLeftCell="A8" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="17"/><cols><col min="1" max="2" width="12" customWidth="1"/><col min="3" max="3" width="20" customWidth="1"/><col min="4" max="7" width="16" customWidth="1"/><col min="8" max="10" width="18" customWidth="1"/></cols><sheetData><row r="1" ht="38"><c r="A1" s="1" t="inlineStr"><is><t>Resultado Renova</t></is></c></row><row r="2" ht="22"><c r="A2" s="2" t="inlineStr"><is><t>Histórico visual de recebimentos e retiradas de aparelhos</t></is></c></row><row r="4" ht="19"><c r="A4" s="3" t="inlineStr"><is><t>TOTAL CADASTRADO</t></is></c><c r="D4" s="3" t="inlineStr"><is><t>AGUARDANDO RETIRADA</t></is></c><c r="G4" s="3" t="inlineStr"><is><t>JÁ RETIRADOS</t></is></c><c r="I4" s="3" t="inlineStr"><is><t>GERADO EM</t></is></c></row><row r="5" ht="33"><c r="A5" s="4" t="n"><v>${items.length}</v></c><c r="D5" s="5" t="n"><v>${awaiting}</v></c><c r="G5" s="6" t="n"><v>${pickedUp}</v></c><c r="I5" s="7" t="inlineStr"><is><t>${xmlCell(generatedOn)}</t></is></c></row><row r="7" ht="27"><c r="A7" s="8" t="inlineStr"><is><t>${items.length} ${items.length === 1 ? 'aparelho encontrado' : 'aparelhos encontrados'}  •  mais recentes primeiro</t></is></c></row>${cards}${emptyRow}</sheetData><mergeCells count="${merges.length}">${mergedCells}</mergeCells><pageMargins left="0.28" right="0.28" top="0.35" bottom="0.35" header="0.2" footer="0.2"/><pageSetup orientation="landscape" paperSize="9" fitToWidth="1" fitToHeight="0"/></worksheet>`;
  const styles = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="9"><font><sz val="10"/><name val="Aptos"/><color rgb="FF2E2834"/></font><font><b/><sz val="18"/><name val="Aptos Display"/><color rgb="FF41215A"/></font><font><sz val="10"/><name val="Aptos"/><color rgb="FF776C80"/></font><font><b/><sz val="8"/><name val="Aptos"/><color rgb="FF76647F"/></font><font><b/><sz val="18"/><name val="Aptos Display"/><color rgb="FF55217A"/></font><font><b/><sz val="18"/><name val="Aptos Display"/><color rgb="FFB26A09"/></font><font><b/><sz val="18"/><name val="Aptos Display"/><color rgb="FF13815A"/></font><font><b/><sz val="10"/><name val="Aptos"/><color rgb="FFFFFFFF"/></font><font><b/><sz val="12"/><name val="Aptos"/><color rgb="FF312738"/></font></fonts><fills count="9"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF3ECF7"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF3DC"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFE5F6EE"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF55217A"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFFFFF"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFBF8FC"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFEDE3F3"/></patternFill></fill></fills><borders count="3"><border><left/><right/><top/><bottom/><diagonal/></border><border><left/><right/><top/><bottom style="thin"><color rgb="FFE4D9E9"/></bottom><diagonal/></border><border><left style="medium"><color rgb="FF8A47BD"/></left><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="16"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" xfId="0" applyFont="1" applyAlignment="1"><alignment vertical="bottom"/></xf><xf numFmtId="0" fontId="2" fillId="0" borderId="1" xfId="0" applyFont="1" applyBorder="1" applyAlignment="1"><alignment vertical="top"/></xf><xf numFmtId="0" fontId="3" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="4" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="5" fillId="3" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="6" fillId="4" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="8" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="7" fillId="5" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" indent="1"/></xf><xf numFmtId="0" fontId="7" fillId="5" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="8" fillId="6" borderId="2" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment vertical="center" indent="1"/></xf><xf numFmtId="0" fontId="3" fillId="3" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="3" fillId="4" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="7" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" indent="1"/></xf><xf numFmtId="0" fontId="2" fillId="6" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1" applyAlignment="1"><alignment vertical="center" indent="1"/></xf><xf numFmtId="0" fontId="2" fillId="0" borderId="0" xfId="0" applyFont="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;
  return zipWorkbook({
    '[Content_Types].xml': `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>`,
    '_rels/.rels': `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`,
    'xl/workbook.xml': `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Resultados Renova" sheetId="1" r:id="rId1"/></sheets></workbook>`,
    'xl/_rels/workbook.xml.rels': `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>`,
    'xl/worksheets/sheet1.xml': sheet,
    'xl/styles.xml': styles,
  });
...
async function exportRenovaSpreadsheet(env) {
  ...
  return new Response(renovaSearchResultsWorkbook(rows, generatedOn), {
    headers: {
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
