...
describe('Controle de estoque por código material', () => {
  ...
  test('atualiza o relatório de 01/09, exclui RPAR e preserva pedidos e chips distribuídos', async () => {
    ...
    assert.equal(Number((await row('SELECT SUM(quantity_on_hand) AS total FROM product_variants')).total), 963);
    assert.equal(Number((await row('SELECT SUM(quantity_on_hand) AS total FROM product_variants WHERE active = 1')).total), 963);
    assert.equal(Number((await row(`SELECT COUNT(*) AS count FROM inventory_serials WHERE status = 'available'`)).count), 963);
    ...
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'inventory_snapshot_date'`)).value, '2026-09-01');
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'inventory_snapshot_source'`)).value, 'ESTOQUE.01.09.xlsx');
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'inventory_snapshot_incoming_depots'`)).value, 'DEPS,NREM');
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'inventory_snapshot_incoming_units'`)).value, '178');
    assert.equal(Number((await row('SELECT COUNT(*) AS count FROM incoming_inventory')).count), 54);
    ...
    assert.equal(Number((await row(`SELECT quantity_on_hand FROM product_variants WHERE sku = '22022526'`)).quantity_on_hand), 13);
    ...
    assert.deepEqual(clusters, {
      ...
      cases: { products: 117, units: 172 },
      chargers: { products: 24, units: 70 },
      devices: { products: 97, units: 139 },
      misc: { products: 49, units: 265 },
      notebooks: { products: 2, units: 1 },
      screen_protectors: { products: 7, units: 230 },
    ...
    assert.equal(Number((await row(`SELECT quantity_on_hand FROM product_variants WHERE sku = 'DGAP22722000'`)).quantity_on_hand), 3);
  ...
  test('configura o gerente e protege a sessão', async () => {
    const health = await manager.request('/api/health');
    assert.equal(health.status, 200);
    assert.equal(health.payload.database, 'available');
  ...
  test('organiza o estoque do painel gerencial nos nove grupos', async () => {
    ...
    assert.equal(dashboard.payload.inventoryGroups.reduce((sum, group) => sum + group.onHand, 0), 963);
    assert.equal(
    ...
    assert.equal(dashboard.payload.management.incomingUnits, 178);
    assert.equal(dashboard.payload.management.incomingMaterials, 54);
    assert.equal(dashboard.payload.management.shortageProducts.length, 12);
    assert.equal(dashboard.payload.management.incomingProducts.length, 54);
    ...
    assert.equal(dashboard.payload.management.snapshot.source, 'ESTOQUE.01.09.xlsx');
    ...
    assert.deepEqual(repairs.payload.summary, { units: 110, materials: 14, snapshotDate: '2026-09-01' });
  ...
  test('expõe o catálogo sem vazar séries e bloqueia movimentação manual', async () => {
    ...
    assert.equal(catalog.payload.products.reduce((sum, product) => sum + product.onHand, 0), 963);
    ...
    assert.equal(catalog.payload.pricing.tableDate, '2026-09-12');
    ...
    assert.equal(sellerCatalog.payload.products.filter((product) => product.pricing).length, 55);
    ...
    assert.equal(newMoto.pricing.prices['FAMILIA 3'], 79900);
    ...
    assert.equal(motoG67.pricing.tableDate, '2026-09-12');
    ...
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'pricing_last_verification_date'`)).value, '2026-09-14');
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'pricing_last_verification_source_table_date'`)).value, '2026-09-12');
    ...
    assert.equal(fold8Catalog.pricing.prices['FAMILIA 3'], 919900);
  ...
  test('mantém os 423 preços de aparelhos iguais à auditoria integral', async () => {
    const source = await readFile(new URL('../scripts/pricing-source-2026-09-12.json', import.meta.url), 'utf8').then(JSON.parse);
    ...
    assert.equal(source.tableDate, '2026-09-12');
  ...
  test('restringe e detalha os produtos a caminho para gerente e estoquista', async () => {
    ...
    assert.deepEqual(managerIncoming.payload.summary, {
      units: 178, materials: 54, snapshotDate: '2026-09-01', source: 'ESTOQUE.01.09.xlsx', status: 'DEPS NREM',
    });
    assert.equal(managerIncoming.payload.products.reduce((sum, product) => sum + product.quantity, 0), 178);
  ...
  test('agrupa aparelhos por modelo e relaciona somente capas compatíveis', async () => {
    ...
    assert.equal(groups.reduce((sum, group) => sum + group.products.length, 0), 70);
  ...
  test('sistema escolhe o IMEI e o gerente pode cancelar devolvendo tudo ao estoque', async () => {
    ...
    assert.deepEqual(created.payload.request.pricing, {
      category: 'VIVO V', deviceTotalCents: 349900, orderTotalCents: 349900, tableDate: '2026-09-12',
    });
  ...
  test('libera conjunto escolhendo aparelho, capa e película de forma automática', async () => {
    ...
    assert.equal(Number((await row(`SELECT quantity_on_hand FROM product_variants WHERE sku = '22023386'`)).quantity_on_hand), 27);
  });
  ...
  test('entrega ao estoquista a operação de estoque e o cancelamento com devolução', async () => {
    ...
    const stockerSerials = await stocker.request('/api/inventory/serials');
    assert.equal(stockerSerials.status, 200);
    assert.ok(stockerSerials.payload.summary.total > 0);
    assert.equal(stockerSerials.payload.summary.total, stockerSerials.payload.items.length);
    assert.ok(stockerSerials.payload.items.every((item) => item.serialNumber && item.productName && item.materialCode));
    assert.equal((await manager.request('/api/inventory/serials')).status, 200);
    assert.equal((await seller.request('/api/inventory/serials')).status, 403);
  ...
  test('faz contagem de estoque exclusiva do gerente, retoma, finaliza sem alterar e aprova com auditoria', async () => {
    ...
    assert.equal(found.headers.get('x-robots-tag'), 'noindex, nofollow');
    assert.equal(found.payload.item.difference, 0);
    assert.equal(found.payload.items, undefined, 'leitura rápida não deve retornar toda a contagem');
    ...
    assert.equal(firstQuickScan.payload.item.countedQuantity, 1);
    assert.equal(firstQuickScan.payload.items, undefined, 'cada leitura retorna somente o item atualizado');
    ...
    assert.equal(secondQuickScan.payload.item.countedQuantity, 2);
  ...
  test('entrega o configurador por modelo sem dependências externas', async () => {
    ...
    assert.equal(JSON.parse(packageSource).version, '6.59.0');
    ...
    assert.match(appSource, /Códigos desta unidade na planilha/);
    ...
    assert.match(appSource, /Parcela aproximada/);
    ...
    assert.match(indexSource, /styles\.css\?v=6\.59\.0/);
    assert.match(indexSource, /app\.js\?v=6\.59\.0/);
    ...
    assert.match(appSource, /Etiquetas da loja/);
    assert.match(appSource, /function storeLabelCatalogRows/);
    assert.match(appSource, /function printableStoreLabel/);
    assert.match(appSource, /const item = visibleLabelRows\(\)\.find\(\(row\) => row\.key === key\)/);
    assert.match(appSource, /Consulte as condições de parcelamento do seu cartão/);
    ...
    assert.match(stylesSource, /font-family:\s*"Vivo Type"/);
    assert.match(stylesSource, /vivo-type-regular\.woff/);
    assert.match(stylesSource, /vivo-type-bold\.woff/);
    ...
    const script = await mf.dispatchFetch('https://controleestoque.app.br/app.js?v=6.59.0');
  ...
  test('atualiza o estoque de 04/09 e destaca os 100 chips recebidos em 03/09', async () => {
    const migrations = await Promise.all([
      readFile(new URL('../migrations/0069_inventory_refresh_2026_09_04.sql', import.meta.url), 'utf8'),
      readFile(new URL('../migrations/0070_incoming_inventory_details_2026_09_04.sql', import.meta.url), 'utf8'),
      readFile(new URL('../migrations/0071_chip_inventory_arrivals_2026_09_03.sql', import.meta.url), 'utf8'),
    ]);
    for (const [migrationIndex, migration] of migrations.entries()) {
      for (const [statementIndex, statement] of migrationStatements(migration).entries()) {
        try { await database.prepare(statement).run(); }
        catch (error) { throw new Error(`Falha na migração ${migrationIndex + 69}, instrução ${statementIndex + 1}: ${statement.slice(0, 100)}`, { cause: error }); }
      }
    }

    assert.equal(Number((await row('SELECT SUM(quantity_on_hand) AS count FROM product_variants WHERE active = 1')).count), 1068);
    assert.equal(Number((await row(`SELECT COUNT(*) AS count FROM inventory_serials WHERE status = 'available'`)).count), 1068);
    assert.equal(Number((await row('SELECT COUNT(*) AS count FROM incoming_inventory_serials')).count), 194);
    assert.equal(Number((await row('SELECT COUNT(*) AS count FROM repair_inventory')).count), 111);
    assert.equal((await row(`SELECT value FROM system_state WHERE key = 'inventory_snapshot_date'`)).value, '2026-09-04');
    assert.equal(Number((await row('SELECT COUNT(*) AS count FROM chip_inventory_arrivals')).count), 100);

    const managerView = await manager.request('/api/chips');
    assert.equal(managerView.status, 200);
    assert.deepEqual(managerView.payload.summary, { available: 0, sold: 0, removed: 0 });
    const latestMaterial = managerView.payload.materials[0];
    assert.equal(latestMaterial.materialCode, 'YBSC001A4000');
    assert.equal(latestMaterial.availableCount, 120);
  ...
  test('atualiza a loja 209H, limpa a contagem solicitada e exclui chips e películas', async () => {
    ...
    assert.equal(scanned.payload.item.countedQuantity, 1);
    assert.equal(scanned.payload.serial.serialNumber, '220233230017303');
    ...
    assert.equal(byMaterial.payload.item.countedQuantity, 2);
